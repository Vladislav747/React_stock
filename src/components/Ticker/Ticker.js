...
export default class Ticker extends React.Component {
    constructor(props) {
        ...
        this.getApiAbbreviationFromCurrency = this.getApiAbbreviationFromCurrency.bind(
            this
        );
    ...
    fetchDataCurrencies(currency) {
        return fetch(
            "https://cors-anywhere.herokuapp.com/https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest?symbol=" +
                currency +
                "&convert=USD",
            {
                headers: {
            ...
            .catch((e) => {
                showNoty(
                    "Произошла ошибка с данными обратитесь к адимнистратору"
                );
            ...
            .then((responseJson) => {
                switch (currency) {
                    case "BCH":
                        return Math.round(
                            responseJson.data.BCH.quote.USD.price - 1
                        );

                    case "LTC":
                        return Math.round(
                            responseJson.data.LTC.quote.USD.price,
                            -1
                        );

                    case "BTC":
                        return Math.round(
                            responseJson.data.BTC.quote.USD.price,
                            -1
                        );

                    case "ETH":
                        return Math.round(
                            responseJson.data.ETH.quote.USD.price,
                            -1
                        );

                    case "XLM":
                        return Math.round(
                            responseJson.data.XLM.quote.USD.price,
                            -1
                        );

                    case "XRP":
                        return Math.round(
                            responseJson.data.XRP.quote.USD.price - 1
                        );
